...
rm(list = ls())
wd <- "~/EROS Anthropocene Paper"
setwd(wd)


###### GRAVITY DATA ######

grav <- read.csv(paste(wd,"/data/totgravity_data.csv", sep=""))
str(grav)
...
fpdat <- read.csv(paste(wd,"/data/FinPrint_reefs_gravity.csv", sep = ""))
str(fpdat)
...
hum <- read.csv(paste(wd,"/humansmaxn.csv", sep = ""), header=FALSE)
non <- read.csv(paste(wd,"/nohumansmaxn.csv", sep = ""), header=FALSE)
